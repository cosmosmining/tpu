...
| 2026-06-07 | E5 | ~~low~~ **RESOLVED** | ~~SPI/CSR top wrapper not integrated.~~ `tt_um_tensortile` now integrates SPI-slave → CSR/bridge (`tt_spi_host`) → descriptor queue + core. End-to-end GEMM over SPI is bit-exact (`tb_top`). | — | done (this commit) |
| 2026-06-07 | E6 | low | **Ping-pong** weight double-buffer evaluated and intentionally NOT shipped: weight load is single-cycle (parallel), so there is no multi-cycle latency to hide, and a 2nd bank worsens area (E1). Dual-bank path verified bit-exact (tb_core PINGPONG=1); load-during-stream overlap had a residual mismatch — reverted vs shipping unverified RTL. | None needed; on-demand `w_req` load. Revisit only with a multi-cycle (shift-chain) load. | deferred (see DECISIONS 2026-06-07) |
